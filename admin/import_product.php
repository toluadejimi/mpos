...
//						}
						...
						$statement = db()->prepare("SELECT * FROM `products` WHERE `p_name` = ? OR `p_code` = ?");
      					$statement->execute(array($pro_data['product_name'], $pro_data['code']));
      					...
      					if (!$product) {
							...
							if ($product_id) {
								foreach ($store_code_names as $store_code) {
			        				...
			        				$statement = $statement->execute(array($product_id, 4, $pro_data['cost_price'], $pro_data['sell_price'], $pro_data['alert_quantity'], 1, $pro_data['brand_id'], $pro_data['box_id'], $pro_data['taxrate_id'], $pro_data['tax_method'], $expired_date, $p_date, $pro_data['status']));
								}
								$insert_status[] = 'ok';
							} else {
								$insert_status[] = 'error';
